// biome-ignore-all lint: this file is not biome compatible
...
export type RenderedFieldMap<SchemaType extends z.ZodObject> = {[key in keyof z.infer<SchemaType["shape"]>]: z.ZodType};


export type RTFFormSchemaType = z.ZodObject<any, any>;
...
type IndexOfUnwrapZodType<T extends RTFSupportedZodTypes> =
  T extends z.core.$ZodBranded<z.ZodType, infer ID> ? ID : T;
...
> = IndexOf<
  ...
  readonly [IndexOfUnwrapZodType<SchemaType>['shape'][key], any]
>;
...
> = [Level] extends [never]
  ...
  : RequireKeysWithRequiredChildren<
      Partial<{
        [key in keyof z.infer<SchemaType>]: GetTupleFromMapping<
          Mapping,
          SchemaType,
          key
        > extends never
          ? SchemaType extends z.ZodObject
				? PropType<
					...
					Prev[Level]
				>
				: SchemaType extends z.ZodArray<any>
				? PropType<
					Mapping,
					SchemaType["_zod"]["def"]["element"],
					PropsMapType,
					Prev[Level]
				>
				: never
...
>(
	...
	return function Component<SchemaType extends RTFFormSchemaType>({
				...
				if (type instanceof z.ZodObject) {
					...
					return Object.entries(shape).reduce((accum, [subKey, subType]) => {
						accum[subKey] = renderComponentForSchemaDeep(
							subType,
							// @ts-expect-error
							props?.[subKey] ? props[subKey] : undefined,
							subKey,
							`${prefixedKey}.${subKey}`,
		...
		function renderFields(
			...
			const shape: Record<string, RTFSupportedZodTypes> = _schema.shape;
			return Object.entries(shape).reduce(
				(accum, [key, type]: [SchemaKey, RTFSupportedZodTypes]) => {
					...
					// @ts-expect-error TS7053: explicit any is fine here
		...
		return (
			<FormProvider {..._form}>
				<ActualFormComponent {...formProps} onSubmit={submitFn}>
					...
					<FormChildren
						// @ts-expect-error we aren't typechecking this field
